...
public class FragmentP817P820 extends FragmentPagina {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        c8_p819_o_EditText.setFilters(new InputFilter[]{new InputFilter.AllCaps()});
        c8_p819_o_EditText.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
                if ((keyEvent.getAction() == KeyEvent.ACTION_DOWN) && (keyCode == KeyEvent.KEYCODE_ENTER)) {
                    ocultarTeclado(c8_p819_o_EditText);
                    m8_p819_linearlayout.requestFocus();
                    return true;
                }
                return false;
            }
        });
        c8_p820_o_EditText.setFilters(new InputFilter[]{new InputFilter.AllCaps()});
        c8_p820_o_EditText.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
                if ((keyEvent.getAction() == KeyEvent.ACTION_DOWN) && (keyCode == KeyEvent.KEYCODE_ENTER)) {
                    ocultarTeclado(c8_p820_o_EditText);
                    m8_p820_linearlayout.requestFocus();
                    return true;
                }
                return false;
            }
